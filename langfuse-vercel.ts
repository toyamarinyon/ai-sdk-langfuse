...
export class LangfuseExporter implements SpanExporter {
  ...
  export(
    ...
    try {
      ...
      for (const span of allSpans) {
        ...
        traceSpanMap.set(
          traceId,
          (traceSpanMap.get(traceId) ?? []).concat(span),
        );
      }
  ...
  private processTraceSpans(traceId: string, spans: ReadableSpan[]): void {
    ...
    this.langfuse.trace({
      ...
      metadata: this.filterTraceAttributes(
        this.parseMetadataTraceAttribute(spans),
      ),
    });

    for (const span of spans) {
      this.logDebug(`span------- ${span}`);
      ...
      modelParameters: {
        toolChoice:
          "ai.prompt.toolChoice" in attributes
            ? attributes["ai.prompt.toolChoice"]?.toString()
            : undefined,
        maxTokens:
          "gen_ai.request.max_tokens" in attributes
            ? attributes["gen_ai.request.max_tokens"]?.toString()
            : undefined,
        finishReason:
        ...
        maxRetries:
          "ai.settings.maxRetries" in attributes
            ? attributes["ai.settings.maxRetries"]?.toString()
            : undefined,
        mode:
          "ai.settings.mode" in attributes
            ? attributes["ai.settings.mode"]?.toString()
            : undefined,
      },
      usage: {
        input:
  ...
  private parseSpanMetadata(
    span: ReadableSpan,
  ): Record<string, (typeof span.attributes)[0]> {
  ...
  private logDebug(message: string, ...args: any[]): void {
    ...
    console.log(
      `[${new Date().toISOString()}] [LangfuseExporter] ${message}`,
      ...args,
    );
  }
  ...
  private parseInput(
    span: ReadableSpan,
  ): (typeof span.attributes)[0] | undefined {
    const attributes = span.attributes;
    const tools =
  ...
  private parseSessionIdTraceAttribute(
    spans: ReadableSpan[],
  ): string | undefined {
    return spans
  ...
  private parseMetadataTraceAttribute(
    spans: ReadableSpan[],
  ): Record<string, any> {
    return spans.reduce(
      (acc, span) => {
